...
# Be aware that spiffcoind and spiffcoin-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in spiffcoind.
#
...
# C locale.
#
# In contrast, spiffcoind does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where spiffcoind is guaranteed to be running
# with the classic locale ("C") whereas the locale of spiffcoin-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in spiffcoind but
# "1,230000" in spiffcoin-qt.
#
...
KNOWN_VIOLATIONS=(
    "src/spiffcoin-tx.cpp.*stoul"
    "src/spiffcoin-tx.cpp.*trim_right"
