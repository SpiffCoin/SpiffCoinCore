# PSBT Howto for Spiff Coin Core

Since Spiff Coin Core 0.17, an RPC interface exists for Partially Signed Spiff Coin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/spiffcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Spiff Coin transactions that are not fully signed
...
Overall, the construction of a fully signed Spiff Coin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Spiff Coin transaction (in network format)
...
#### Multisig with multiple Spiff Coin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Spiff Coin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `spiffcoin-cli` is used.
...
Later, when *V* SPC has been received on *Amulti*, and Bob and Carol want to
