...
Bitcoin Core version 0.20.0 is now available from:
  ...
  <https://spiffcoin.net/bin/spiffcoin-core-0.20.0/>
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/spiffcoin/spiffcoin/issues/16599),
  [PR #16702](https://github.com/spiffcoin/spiffcoin/pull/16702), and the
  `spiffcoind help` for more information.  This option is experimental and
...
- A "known bug" [announced](https://spiffcoin.net/en/releases/0.18.0/#wallet-gui)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  program such as [HWI](https://github.com/spiffcoin-core/HWI) for
...
- Bitcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.spiffcoin.net.  (#17596)
...
- The `spiffcoin-cli` utility used with the `-getinfo` parameter now
...
- The `spiffcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `spiffcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_spiffcoin-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_spiffcoin` binary (hebasto)
...
- #18702 Fix ASLR for spiffcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate spiffcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest spiffcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_spiffcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (spiffcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone spiffcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #17119 Fix broken spiffcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from spiffcoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.spiffcoin.net (laanwj)
...
- #17648 Rename wallet-tool references to spiffcoin-wallet (hel-o)
...
- #18341 Replace remaining literal SPC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On spiffcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from spiffcoin-cli -getinfo (malevolent)
