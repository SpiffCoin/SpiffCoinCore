...
Bitcoin Core version 0.21.0 is now available from:
  ...
  <https://spiffcoin.net/bin/spiffcoin-core-0.21.0/>
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/spiffcoin/bips/blob/master/bip-0155.mediawiki).
...
- This release adds support for serving
  [BIP157](https://github.com/spiffcoin/bips/blob/master/bip-0157.mediawiki) compact
...
- This release adds support for signets
  ([BIP325](https://github.com/spiffcoin/bips/blob/master/bip-0325.mediawiki)) in
...
- This release implements
  [BIP339](https://github.com/spiffcoin/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/spiffcoin/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/spiffcoin/bips/blob/master/bip-0342.mediawiki)),
...
- A new `spiffcoin-cli -netinfo` command provides a network peer connections
...
- A new `spiffcoin-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `spiffcoin-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `spiffcoin-cli -getinfo` is now expanded to return a JSON
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `spiffcoin-wallet` tool which performs the salvage
...
`spiffcoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Bitcoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `spiffcoin-cli createwallet` or `spiffcoin-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/spiffcoin/spiffcoin/issues).
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in SPC per 1,000
  vbytes (SPC/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from SPC/kB
  to sat/vB and the help documentation in SPC/kB is updated to SPC/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from SPC/kvB to sat/vB and the
  ...
  difference between SPC/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in SPC/kvB rather than sat/vB should raise an
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  ...
  `\<datadir\>/settings.json` and augments any command line or `spiffcoin.conf`
...
- #19284 Add seed.spiffcoin.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in spiffcoin-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add spiffcoin-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in spiffcoin-cli (laanwj)
...
### Build system
- #18504 Drop spiffcoin-tx and spiffcoin-wallet dependencies on libevent (ryanofsky)
...
- #18653 add coverage for spiffcoin-cli -rpcwait (jonatack)
...
- #19061 Add link to Visual Studio build readme (maitrespiffcoin)
...
- #18817 Document differences in spiffcoind and spiffcoin-qt locale handling (practicalswift)
...
[Transifex](https://www.transifex.com/spiffcoin/spiffcoin/).
