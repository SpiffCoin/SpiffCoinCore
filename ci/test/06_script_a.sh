...
export P_CI_DIR="${BASE_BUILD_DIR}/spiffcoin-$HOST"
...
if [[ ${USE_MEMORY_SANITIZER} == "true" ]]; then
  ...
  # details.
  DOCKER_EXEC 'grep -v HAVE_SYS_GETRANDOM src/config/spiffcoin-config.h > src/config/spiffcoin-config.h.tmp && mv src/config/spiffcoin-config.h.tmp src/config/spiffcoin-config.h'
fi
