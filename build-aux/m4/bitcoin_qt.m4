...
dnl Output: If qt version is auto, set spiffcoin_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$spiffcoin_qt_want_version" = xauto && test "x$spiffcoin_qt_force" != xyes; then
    if test "x$spiffcoin_enable_qt" != xno; then
      AC_MSG_WARN([$1; spiffcoin-qt frontend will not be built])
    fi
    spiffcoin_enable_qt=no
    spiffcoin_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$spiffcoin_enable_qt" != xno && test "x$spiffcoin_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build spiffcoin-qt GUI (default=auto)])],
    [
     spiffcoin_qt_want_version=$withval
     if test "x$spiffcoin_qt_want_version" = xyes; then
       spiffcoin_qt_force=yes
       spiffcoin_qt_want_version=auto
     fi
    ],
    [spiffcoin_qt_want_version=auto])
...
dnl Outputs: spiffcoin_enable_qt, spiffcoin_enable_qt_dbus, spiffcoin_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$spiffcoin_cv_static_qt" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    spiffcoin_enable_qt=yes
    spiffcoin_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      spiffcoin_enable_qt_test=no
    fi
    spiffcoin_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      spiffcoin_enable_qt_dbus=yes
    fi
  ...
  if test x$spiffcoin_enable_qt = xyes; then
    AC_MSG_RESULT([$spiffcoin_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$spiffcoin_enable_qt])
  fi
...
dnl Output: spiffcoin_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, spiffcoin_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [spiffcoin_cv_static_qt=yes],
      [spiffcoin_cv_static_qt=no])
    ])
])
